 ...
 * Zip压缩类
 *
 * @version        $id:zip.class.php 2 13:53 2010-11-11 tianya $
...
class zip
{
    ...
    {
        if (!is_array($files) || (is_array($files) && !isset($files[0]))) {
            $files = array($files);
        }
        if (!is_array($files[0])) {
            $files = array($files);
    ...
    function add_File($data, $name, $compact = 1)
    {
        ...
        $hexdtime = chr(hexdec(substr($dtime, 6, 2))).chr(hexdec(substr($dtime, 4, 2))).chr(hexdec(substr($dtime, 2, 2))).chr(hexdec(substr($dtime, 0, 2)));
        if ($compact) {
    ...
    /**
     *  解压整个压缩包
     *  直接用Extract会有路径问题，本函数先从列表中获得文件信息并创建好所有目录然后才运行Extract
     *
     * @access    public
