...
{
    define('TOOLS_ALERT_TPL', '<div style="position:relative;padding:1rem;margin-bottom:1rem;width:auto;font-size:14px;color:~color~;background:~background~;border-color:~border~;border:1px solid transparent;border-radius:0.5rem">~content~</div>');
...
if (!file_exists(dirname(__FILE__).'/system/common.inc.php')) {
    echo ToolAlert("请把resetpwd.php文件放置网站根目录，通过：域名/resetpwd.php访问进行操作");
    exit();
}
...
<tr>
    <td>管理员新密码验证</td>
    <td><input type="password" name="renewpwd" id="renewpwd" class="admin-input-lg" placeholder="请再次输入管理员密码验证"></td>
</tr>
<tr>
    <td>数据库密码</td>
    <td><input type="password" name="dbpwd" id="dbpwd" class="admin-input-lg" placeholder="请输入数据库密码"></td>
</tr>');
